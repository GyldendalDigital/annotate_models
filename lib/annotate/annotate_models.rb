...
#
# ---
#  Legend: (--> belongs_to) (<-- has_one)  (<== has_many)
#
        EOS
      else
        assoc_header = <<-EOS
...
#
      ...
      associations.sort_by{|name,assoc| name}.each do |name,assoc|
        other_model_name = if assoc.options.has_key?(:through) then
                             assoc.options[:source].try(:to_s).try(:classify)
                           else
                             assoc.options[:class_name].try(:to_s)
                           end
        # If there was no explicit class provided, then try fallbacks for class lookup.
        if other_model_name.blank? then
          begin
            other_model_name = assoc.class_name
          rescue # Lookup of class_name can give a traceback, instead of nil, and in that case, try with the name.
            other_model_name = name.to_s.classify
          end
        end
        ...
        begin
          ...
          if assoc.options.has_key?(:through) then
